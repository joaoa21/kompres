...
(() => {
  ...
  document.querySelectorAll('select:not([multiple])').forEach(select => {
    ...
    const trigger = document.createElement('button'); trigger.type = 'button'; trigger.className = 'kompres-select-trigger'; trigger.id = `${select.id}-trigger`;
    ...
    const menu = document.createElement('ul'); menu.className = 'kompres-select-menu'; menu.id = `${select.id}-menu`; menu.setAttribute('role', 'listbox'); menu.setAttribute('aria-hidden', 'true');
    if (label) menu.setAttribute('aria-labelledby', label.id);
    if (select.hasAttribute('aria-label')) { trigger.setAttribute('aria-label', select.getAttribute('aria-label')); menu.setAttribute('aria-label', select.getAttribute('aria-label')); }
    trigger.setAttribute('aria-controls', menu.id);
    let open = false, active = select.selectedIndex, query = '', lastKey = 0;
    const options = Array.from(select.options, (option, index) => {
      const row = document.createElement('li'); row.className = 'kompres-select-option'; row.id = `${select.id}-option-${index}`; row.setAttribute('role', 'option'); row.textContent = option.textContent;
